const moods = [
  { value: "calm", label: "平静", icon: "○", tone: "像缓慢退潮后的海面", color: "#8d91de" },
  ...
  { value: "warm", label: "温暖", icon: "♡", tone: "像掌心里未熄灭的小火", color: "#d59778" },
...
function buildSignature(emotion) {
  ...
  return Object.assign({}, base, {
    ...
    summary: `${mood.label} / ${need.label} / ${signal.label} / ${intensity}Hz`,
    strength: intensity >= 8 ? "强烈而清晰" : intensity <= 3 ? "轻柔而隐约" : "稳定地浮现",
    insight
  });
}
...
function buildMoodMap(emotion) {
  ...
  return {
    headline: `全球有 ${totalPeople.toLocaleString()} 个匿名信号正在亮起`,
...
function buildMemory(emotion, person) {
  ...
  return {
    ...
    shareTitle: "一张来自 Mood Earth 的同频记忆卡",
    shareText
  };
}
